    ...
    signal = FALSE, dist.epsilon = 1e-10, messages)
  {
    ...
    if (nwin == "full") {
      ...
      if (m0 != "kt") {
        ...
        if (m0 == "kt" & trend == 1) {
          if (d == 1) {
            ...
            xmat <- cbind(rep(1, n), coords[, 1], coords[,
              2])
            xmati <- cbind(rep(1, ni), locations[, 1],
              locations[, 2])
          }
        ...
        if (m0 == "kt" & trend == 2) {
          if (d == 1) {
            ...
            xmat <- cbind(rep(1, n), coords[, 1], coords[,
              2], (coords[, 1])^2, (coords[, 2])^2, coords[,
                1] * coords[, 2])
            xmati <- cbind(rep(1, ni), locations[, 1],
              locations[, 2], (locations[, 1])^2, (locations[,
                2])^2, locations[, 1] * locations[, 2])
          }
          iviv <- solve(crossprod(xmat, iv) %*% xmat)
          txiv <- crossprod(xmat, iv)
          beta <- iviv %*% txiv %*% data
      ...
      for (i in 1:ni) {
        if (messages.screen) {
          if (ni < 11)
            cat(paste("ksline: kriging location: ", i,
              "out of", ni, "\n"))
          else {
            if (ni < 101 & (i%%10 == 1))
              cat(paste("ksline: kriging location: ",
                i, "out of", ni, "\n"))
            if (ni > 100 & i%%100 == 1)
              cat(paste("ksline: kriging location: ",
                i, "out of", ni, "\n"))
        ...
        coords0 <- cbind((coords.c[, 1] - locations.c[i,
          1]), (coords.c[, 2] - locations.c[i, 2]))
        ...
        if (m0 == "ok") {
          ...
          if (signal == TRUE)
            kvar[i] <- sigmasq - v0ivv0 + (redu %*% den %*%
                redu)
          else kvar[i] <- tausq + sigmasq - v0ivv0 + (redu %*%
              den %*% redu)
      ...
      if (messages.screen)
        cat(paste(message, "\n"))
      results <- list(predict = est, krige.var = kvar, dif = dif,
      ...
      if (m0 == "kt") {
        if (trend == 1) {
          if (d == 1)
            xmati <- cbind(rep(1, ni), locations[, 2])
          else xmati <- cbind(rep(1, ni), locations[,
            1], locations[, 2])
        }
        if (trend == 2) {
          if (d == 1)
            ...
            1], locations[, 2], (locations[, 1])^2, (locations[,
              2])^2, locations[, 1] * locations[, 2])
        }
        betawin <- matrix(0, nrow = (ncol(xmati) * ni),
          ncol = ncol(xmati))
      }
      if (m0 == "kte") {
        xmati <- cbind(rep(1, ni), ktelocations)
      ...
      for (i in 1:ni) {
        temp.win <- .ksline.aux.1(coords = coords, coords.c = coords.c,
          data = data, n = n, locations = locations[i,
            ], locations.c = locations.c[i, ], cov.pars = cov.pars,
      ...
      if (messages.screen)
        cat(paste(message, "\n"))
      results <- list(predict = estwin, krige.var = kvarwin,
    ...
    if (abs(lambda - 1) > 0.001) {
      ...
      if (lambda > 0.001) {
        ...
        temp.data <- suppressWarnings(matrix(rnorm(ni *
            n.samples.backtransform, mean = results$predict,
          sd = sqrt(results$krige.var)), nrow = ni))
        ...
        results$krige.var <- as.vector(apply(temp.data,
          1, var))
      }
      if (lambda < -0.001) {
