...
This article shows how to krig soil data based on code by Graham Zemunik. Here you'll learn about `krig()` and `krig_lst()`, which wrap the original function `GetKrigedSoil()` and provide the same result plus several convenient features. To learn more about these functions see `?krig()`.
...
```{r}
# krig_lst() -- but not krig() -- can take multiple soil variables.
