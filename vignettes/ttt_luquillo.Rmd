...
This article shows how to determine habitat-species associations with the function [`torusonesp.all()`](https://forestgeo.github.io/fgeo.habitat/reference/torusonesp.all.html), developed by Sabrina Russo, Daniel Zuleta, Matteo Detto, and Kyle Harms. 
...
We can wrap the code to run the Torus Translation Test into a convenient function, `ttt()`, to abstract out the boilerplate code and focus on what matters most.

```{r}
ttt_test <- function(sp, ...) {
...
The output of `ttt_test()` doesn't scale up very well. With increasing number of species the resulting matrix becomes harder to visualize.
...
```{r}
ttt_gather <- function(ttt_test) {
  ttt_test %>% 
    as.data.frame() %>% 
    add_column(metric = rownames(ttt_test)) %>% 
    gather(key = "sp", value = "value", -metric)
}

result_long <- ttt_gather(result_bigger)
```

## Data science workflows

Above we used `ttt_gather()` to output a dataframe. Compared to a matrix, a dataframe fits better in common data science workflows. Dataframes allow using popular tools for data wrangling and visualization (from the __dplyr__ and __ggplot2__ packages). Here are some examples of what you can do.
