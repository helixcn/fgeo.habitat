...
This article shows how to determine habitat-species associations with the function [`tt_test()`](https://forestgeo.github.io/fgeo.habitat/reference/tt_test_one.html), developed by Sabrina Russo, Daniel Zuleta, Matteo Detto, and Kyle Harms. 
...
### With `tt_test_lst()` and any number of species

```{r}
all_sp <- unique(pick$sp)
out <- tt_test_lst(census, all_sp, habitat)
head(out)
```
...
```{r}
spread(as_df(out), sp, value)
```

### With `tt_test()` and one species

Before we used `tt_test_lst()`, which is a wrapper around `tt_test()` -- the original function developed by Sabrina Russo, Daniel Zuleta, Matteo Detto, and Kyle Harms. This section shows how you may use `tt_test()` directly to test a single species. The following section shows how to extend the test to any number of species.

See `?tt_test()`; you'll find out that this function needs the name of one species, a census, and habitat data. All of these we already have (above). But `tt_test()` also needs plot dimensions, grid size and the abundance (count) of each species per quadrat-index.
...
Now that we have all we need, we can run `tt_test()`.

```{r}
one_species <- unique(pick$sp)[[1]]
one_species

out_one <- tt_test(one_species, habitat, abundance, plot_dimensions, grid_size)
out_one
```

You may transform the output into a dataframe with `as_df()`.
...
out_all <- map(
  all_species, 
  tt_test, habitat, abundance, plot_dimensions, grid_size
)
out_all
```

Getting a useful dataframe now takes a bit more work. Again, you may use `as_df()` to create a dataframe for each element of the list, then you can reduce the list to a single dataframe by `r`ow`bind()`ing each dataframe into a single one with `reduce()`.

```{r}
df_lst <- map(out_all, as_df)
tt_df <- reduce(df_lst, rbind)
head(tt_df)
```

But this data structure is defined as of class "tt_lst", which `as_df()` understands. Therefore you can achieve all of the above by explicitely assigning that class. 

```{r}
class(out_all) <- "tt_lst"
tt_df <- as_df(out_all)
head(tt_df)
```

Finally, you can visualize this data with `View()` or `spread()`.
...
Above we used `tt_test_lst()`, or a combination of `tt_test()` and `as_df()`, to output a dataframe. Compared to a matrix, a dataframe fits better in common workflows for data manipulation and visualization. The dataframe is the most important data structure used in __dplyr__, __ggplot2__, and all other packages in the __tidyverse__. Here are some examples of what you can do with our dataframe output.

The next few code chunks use the pipe operator (`%>%`) to avoid saving intermediary results and to make our code more expressive -- where each line is an imperative statement that communicates our intention (for details see http://style.tidyverse.org/pipes.html).
